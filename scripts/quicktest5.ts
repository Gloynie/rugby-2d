...
(globalThis as any).document = { createElement: (tag: string) => { if (tag !== "canvas") throw new Error("unexpected"); const c: any = { width: 0, height: 0, style: {} }; c.getContext = () => ctx; return c; }, fonts: { load: () => Promise.resolve([]) } };
...
async function run() {
  ...
  // 90s real time = enough for full match
  while (!finished && frames < 5400) {
    perfNow = frames * (1000 / 60);
    raf(performance.now());
    scenes.add(rt.director.scene);
    frames++;
    if (frames === 600) console.log("t=10s scene=", rt.director.scene, "phase=", rt.engine.phase);
    if (frames === 1800) console.log("t=30s scene=", rt.director.scene, "phase=", rt.engine.phase);
    if (frames === 3600) console.log("t=60s scene=", rt.director.scene, "phase=", rt.engine.phase);
  }
  console.log("frames=", frames, "scenes:", [...scenes].join(","), "engine phase=", rt.engine.phase);
